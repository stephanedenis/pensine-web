...
/**
 * Helper: store typed values in localStorage
 */
function writeLocalConfig(key, value) {
    try {
        localStorage.setItem(key, JSON.stringify(value));
    } catch (error) {
        console.warn('[Config] Unable to persist value', key, error);
    }
}

function readLocalConfig(key, fallback = null) {
...
class ConfigBridge {
    constructor(modernConfigManager) {
    ...
    getFromLocalStorage() {
        const storedWeekStart = readLocalConfig('weekStartDay', UI_CONFIG_DEFAULTS.weekStartDay);

        return {
            calendarVisible: readLocalConfig('calendarVisible', UI_CONFIG_DEFAULTS.calendarVisible) !== false,
            historyVisible: readLocalConfig('historyVisible', UI_CONFIG_DEFAULTS.historyVisible) === true,
            weekStartDay: Number.isInteger(storedWeekStart)
                ? storedWeekStart
                : parseInt(storedWeekStart, 10) || UI_CONFIG_DEFAULTS.weekStartDay,
            theme: readLocalConfig('theme', UI_CONFIG_DEFAULTS.theme) || UI_CONFIG_DEFAULTS.theme
        };
    }

    async loadFromGitHub() {
        // Prefer modern ConfigManager when available
        if (this.modernConfigManager) {
            try {
                if (typeof this.modernConfigManager.init === 'function') {
                    await this.modernConfigManager.init();
                }

                const coreConfig = typeof this.modernConfigManager.getCoreConfig === 'function'
                    ? this.modernConfigManager.getCoreConfig()
                    : {};

                const merged = { ...UI_CONFIG_DEFAULTS, ...coreConfig };
                this.persistLocal(merged);
                return merged;
            } catch (error) {
                console.warn('[ConfigBridge] Modern config unavailable, falling back:', error);
            }
        }

        // Fallback to legacy GitHub adapter if available
        try {
            const { config, sha } = await githubAdapter.getConfig();
            this.configSha = sha;
            const merged = { ...UI_CONFIG_DEFAULTS, ...config };
            this.persistLocal(merged);
            return merged;
        } catch (error) {
            console.warn('[ConfigBridge] Could not load config from GitHub, using localStorage:', error);
            return { ...UI_CONFIG_DEFAULTS, ...this.getFromLocalStorage() };
        }
    }
    ...
    async saveToGitHub(key, value) {
        ...
        if (this.modernConfigManager) {
            try {
                const coreConfig = typeof this.modernConfigManager.getCoreConfig === 'function'
                    ? this.modernConfigManager.getCoreConfig()
                    : {};
                await this.modernConfigManager.setCoreConfig({ ...coreConfig, [key]: value });
                return;
            } catch (error) {
                console.error('[ConfigBridge] Failed to save via modern config manager:', error);
            }
        }
        ...
        try {
            ...
            console.error('[ConfigBridge] Could not save config to GitHub:', error);
        }
    }
...
class PensineApp {
    constructor() {
        ...
        // Config bridge (updated after modern config init)
        this.configManager = new ConfigBridge(null);
    ...
    async init() {
        ...
        try {
            ...
            const { initializeModernConfig } = await import('./lib/components/settings-integration.js');
            ...
            window.modernConfigManager = modernConfigManager;
        ...
        // Bridge config usage to the modern manager when available
        this.configManager = new ConfigBridge(this.modernConfigManager);
    ...
    async restorePanelStates() {
        ...
        const config = await this.configManager.loadFromGitHub();
        ...
        if (config.calendarVisible && calendarContainer) {
            if (sidebarNav) {
                sidebarNav.classList.add('hidden');
            }
            await this.initCalendar();
        } else if (calendarContainer && recentPages) {
            calendarContainer.classList.add('hidden');
            recentPages.classList.remove('hidden');
        }

        // Restore history panel state
        if (config.historyVisible && historySidebar) {
    ...
    setupPanelResize() {
        ...
        const savedWidth = readLocalConfig('sidebar-width');
        if (savedWidth) {
            const widthValue = Number(savedWidth);
            if (Number.isFinite(widthValue)) {
                document.documentElement.style.setProperty('--sidebar-width', widthValue + 'px');
            }
        }
        ...
        document.addEventListener('mouseup', () => {
            if (isResizing) {
                ...
                writeLocalConfig('sidebar-width', currentWidth);
            }
        });
    ...
    switchView(viewName) {
        ...
        if (!views || views.length === 0) {
            console.warn('No views available to switch');
            return;
        }
        ...
        if (!targetView) {
            console.warn('Target view not found', viewName);
            return;
        }
    ...
    async manualSync() {
        try {
            ...
            const hasConflict = syncStatus ? syncStatus.classList.contains('conflict') : false;
    ...
    updateSyncStatus(status) {
        ...
        if (!indicator) {
            console.warn('Sync status indicator not found');
            return;
        }
    ...
    async toggleHistory() {
        ...
        if (!sidebar) {
            console.warn('History sidebar not found');
            return;
        }
        ...
        await this.configManager.saveToGitHub('historyVisible', isOpen);
    }
    ...
    async closeHistory() {
        ...
        await this.configManager.saveToGitHub('historyVisible', false);
    }
    ...
    showCalendarConfig() {
        ...
        setTimeout(() => {
            const weekStartEl = document.getElementById('config-weekStartDay');
            const monthFormatEl = document.getElementById('config-monthFormat');
            const dayNumberPosEl = document.getElementById('config-dayNumberPosition');
            const dayHeightEl = document.getElementById('config-dayHeight');
            const monthColorsEl = document.getElementById('config-monthColors');

            if (weekStartEl) weekStartEl.value = weekStartDay;
            if (monthFormatEl) monthFormatEl.value = monthFormat;
            if (dayNumberPosEl) dayNumberPosEl.value = dayNumberPosition;
            if (dayHeightEl) dayHeightEl.value = dayHeight;
            if (monthColorsEl) monthColorsEl.checked = monthColors;
        }, 0);
    }
    ...
    async applyCalendarConfig() {
        ...
        const weekStartDayEl = document.getElementById('config-weekStartDay');
        const monthFormatEl = document.getElementById('config-monthFormat');
        const dayNumberPositionEl = document.getElementById('config-dayNumberPosition');
        const dayHeightEl = document.getElementById('config-dayHeight');
        const monthColorsEl = document.getElementById('config-monthColors');

        if (!weekStartDayEl || !monthFormatEl || !dayNumberPositionEl || !dayHeightEl || !monthColorsEl) {
            console.warn('Calendar config form elements missing');
            return;
        }

        const parsedWeekStart = parseInt(weekStartDayEl.value, 10);
        const weekStartDay = Number.isInteger(parsedWeekStart) ? parsedWeekStart : UI_CONFIG_DEFAULTS.weekStartDay;
        const monthFormat = monthFormatEl.value;
        const dayNumberPosition = dayNumberPositionEl.value;
        const parsedDayHeight = parseInt(dayHeightEl.value, 10);
        const dayHeight = Number.isFinite(parsedDayHeight) ? parsedDayHeight : 28;
        const monthColors = monthColorsEl.checked;
        ...
        // Save to localStorage with preserved types
        writeLocalConfig('weekStartDay', weekStartDay);
        writeLocalConfig('monthFormat', monthFormat);
        writeLocalConfig('dayNumberPosition', dayNumberPosition);
        writeLocalConfig('dayHeight', dayHeight);
        writeLocalConfig('monthColors', monthColors);
    ...
    async toggleCalendar() {
        ...
        if (!container || !recentPages) {
            console.warn('Calendar container or recent pages panel missing');
            return;
        }
        ...
        if (container.classList.contains('hidden')) {
            ...
            await this.configManager.saveToGitHub('calendarVisible', true);
            ...
            await this.configManager.saveToGitHub('calendarVisible', false);
        }
    }
    ...
    async initCalendar() {
        ...
        const monthFormat = readLocalConfig('monthFormat', 'short') || 'short';
        const dayNumberPosition = readLocalConfig('dayNumberPosition', 'center') || 'center';
    ...
    updateConfigFromForm(originalConfig) {
        try {
            const form = document.getElementById('config-editor-form');
            if (!form) {
                console.warn('Config editor form not found');
                return;
            }
    ...
    async openInEditor(path, content) {
        ...
        if (!editorContainer || !fileNameEl || !fileTypeBadge || !codeTextarea || !richContent) {
            console.warn('Editor elements missing, cannot open file');
            return;
        }
        ...
        if (this.currentFileType === FILE_TYPES.CONFIG) {
            ...
            initialMode = readLocalConfig('editorViewMode', VIEW_MODES.RICH) || VIEW_MODES.RICH;
        }
    ...
    switchEditorMode(mode) {
        ...
        if (!editorContainer) {
            console.warn('Editor container not found for mode switch');
            return;
        }
        ...
        // Save preference
        writeLocalConfig('editorViewMode', mode);
