...
class ConfigWizard {
    ...
    async complete() {
        try {
            ...
            // Initialize GitHubAdapter with new config
            ...
            // Create .pensine-config.json file
            await githubAdapter.createFile('.pensine-config.json', configContent, 'Initial Pensine configuration');

            // Save to localStorage as well
            localStorage.setItem('pensine-config', configContent);
            localStorage.setItem('github-token', this.config.git.token);
            ...
            // Hide wizard and reload app
            this.hide();
            
            // Reload the app with new config
            if (window.app) {
