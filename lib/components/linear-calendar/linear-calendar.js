...
class LinearCalendar extends ConfigurableComponent {
  ...
  generateWeekRow(weekStart) {
    ...
    // Month start marker - check if this week contains day 1 of ANY month
    let monthStartValue = null;
    for (let i = 0; i < 7; i++) {
      const dayDate = new Date(weekStart);
      dayDate.setDate(dayDate.getDate() + i);
      if (dayDate.getDate() === 1) {
        // This week contains the 1st of a month - mark it
        monthStartValue = dayDate.getMonth();
        break;
      }
    }

    // Mark as month-start if this week contains the first day of any month
    if (monthStartValue !== null) {
      weekDiv.classList.add('month-start');
      weekDiv.dataset.monthStart = monthStartValue;
    }
    ...
    // Add month label as 8th flex item for month-start weeks
    // Append AFTER all 7 day cells are added
    if (weekDiv.classList.contains('month-start') && monthStartValue !== null) {
      const monthLabel = document.createElement('div');
      monthLabel.className = 'calendar-month-label';

      // Find the actual first day of the month to get correct year
      let monthDate = null;
      for (let i = 0; i < 7; i++) {
        const dayDate = new Date(weekStart);
        dayDate.setDate(dayDate.getDate() + i);
      ...
      if (!monthDate) {
        // Fallback (shouldn't happen for month-start weeks)
        monthDate = new Date(weekStart);
        monthDate.setMonth(monthStartValue);
      }
      ...
      // December and January: show year suffix
